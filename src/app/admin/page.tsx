...
export default function AdminPage() {
	...
	const handleStatusToggle = async (id: string, currentlyActive: boolean) => {
		if (togglingId) return;
		setTogglingId(id);
		try {
			await users.updateStatus(id, { isActive: !currentlyActive });
			setUserList((prev) =>
				prev.map((u) =>
								...
								{userList.length === 0 ? (
									...
									userList.map((u) => (
										<tr
											...
											<td className="px-4 py-3">
												<select
													className="text-xs border border-gray-200 rounded-md px-2 py-1 bg-white focus:outline-none focus:ring-1 focus:ring-violet-500 disabled:opacity-50"
													value={u.role}
